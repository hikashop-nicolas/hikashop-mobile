...
describe('locales', () => {
	...
	it('uses the plural forms its language actually has', async () => {
		...
		for (const tag of ['pl-PL', 'ru-RU', 'cs-CZ', 'sk-SK', 'ro-RO', 'he-IL', 'ar-AA', 'hr-HR', 'sl-SI', 'lt-LT', 'lv-LV', 'uk-UA']) {
			...
			const cats = [...new Set([0, 1, 2, 3, 5, 11, 21, 22, 100].map((n) => new Intl.PluralRules(tag).select(n)))];
			let distinguishes = 0;
